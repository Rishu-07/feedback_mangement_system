...
@app.route('/not_saturday')
def not_saturday():
    user_info = session.get('user_info')

    if not user_info or not re.match(r'^su-230.*@sitare\.org$', user_info.get('email', '')):
        print("User not authorized for teacher portal. Redirecting to login.")
        return redirect(url_for('login'))
...
def teacher_portal():
    user_info = session.get('user_info')
    print("Accessing teacher portal for user:", user_info)
    ...
    try:
        ...
        if conn:
            ...
            cursor.close()
            conn.close()
            print("Feedback data fetched for instructor:", instructor_email)
        else:
            print("Failed to fetch feedback data due to connection issue.")
            feedback_data = []
    except psycopg2.Error as e:
    ...
    return render_template('teacher_portal.html', user_info=user_info, feedback_data=feedback_data)
...
def admin_portal():
    user_info = session.get('user_info')
    print("Accessing admin portal for user:", user_info)

    if not user_info or user_info['email'] != 'admin@sitare.org':
        print("User not authorized for admin portal. Redirecting to login.")
        return redirect(url_for('login'))
...
def submit_all_forms():
    ...
    for course_id, form_data in feedback_entries.items():
        ...
        prepared_feedback_entries.append(
            (course_id, student_email_id, date_of_feedback, current_week_no, instructor, understanding_rating, revision_rating, suggestion)
        )
