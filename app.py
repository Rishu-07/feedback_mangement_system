...
def student_portal():
    ...
    emails = {
        "Dr. Kushal Shah": "kushal@example.com",
        "Dr. Sonika Thakral": "sonika@example.com",
        "Dr. Achal Agrawal": "achal@example.com",
        "Ms. Preeti Shukla": "preet@example.com",
    ...
    return render_template('student_portal.html', user_info=user_info, courses=courses, instructor_emails = Email)
...
def submit_all_forms():
    ...
    week = datetime.isoweekday()
    # student_email = user.email()

    for course_key, form_data in data.items():
        course_id = course_key.split('_')[1]
        understanding_rating = form_data.get(f'understanding', [None])[0]
        revision_rating = form_data.get(f'revision', [None])[0]
        remarks = form_data.get(f'remarks', [''])[0]
    ...
    return jsonify({"status": "success", "message": "Your feedback was submitted successfully!"})
